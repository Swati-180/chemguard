...
export default function LoginPage() {
  ...
  const initializeDemoAccounts = async () => {
    ...
    try {
      for (const account of accounts) {
        ...
        if (uid) {
          // 3. Sync profile to Firestore collection "users"
  ...
  // Auto-initialize demo accounts on first visit
  React.useEffect(() => {
  ...
  }, [user, isUserLoading, isInitializing, isVerifying, initComplete, db, router, auth])
      ...
      <div className="min-h-screen w-full bg-[#0a0f18] flex flex-col items-center justify-center p-6 relative overflow-hidden text-white">
        <div className="absolute inset-0 opacity-10 pointer-events-none">
          <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id="hex-load" width="50" height="43.4" patternUnits="userSpaceOnUse" patternTransform="scale(2)">
        ...
        <div className="flex flex-col items-center gap-6 relative z-10">
          ...
          <div className="text-center space-y-2">
            <p className="text-[10px] font-bold text-primary uppercase tracking-[0.4em] animate-pulse">
              {isInitializing ? "Provisioning Security Nodes" : "Establishing Secure Satellite Link"}
            </p>
            <p className="text-[8px] text-muted-foreground uppercase tracking-widest opacity-50">
              ChemGuard AI SOC Console v4.2.1
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen w-full bg-[#0a0f18] flex flex-col items-center justify-center p-6 relative overflow-hidden text-white">
      ...
      <header className="text-center space-y-2 mb-12 relative z-10">
        <div className="flex items-center justify-center gap-4 mb-4">
          ...
          <h1 className="text-4xl font-headline font-bold tracking-tight text-white uppercase tracking-tighter">
            ChemGuard <span className="text-primary">AI</span> Control System
          </h1>
        </div>
        <p className="text-muted-foreground uppercase tracking-[0.3em] text-xs font-medium">Secure Chemical Supply Chain Monitoring & SOC</p>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full max-w-7xl relative z-10">
        <PortalCard 
          title="Admin Portal"
          description="Global system monitoring and fleet management console."
          icon={Settings}
          color="primary"
          badges={["ISO 27001 Certified", "Root Admin"]}
        ...
        <PortalCard 
          ...
          description="Laboratory chemical inventory and usage tracking."
          icon={Beaker}
          color="accent"
        ...
        <PortalCard 
          ...
          description="Real-time shipment tracking and delivery validation."
          icon={Truck}
          color="orange"
          badges={["GPS Validated", "Fleet Logistics"]}
      ...
      <div className="mt-12 flex flex-col items-center gap-4 relative z-10">
        <div className="p-4 bg-white/5 border border-white/10 rounded-2xl max-w-md text-center backdrop-blur-sm">
          ...
          <p className="text-[11px] text-white/60 leading-relaxed mb-4">
            If the system is not yet initialized, click below to provision the secure demo environment.
          </p>
          <Button 
            variant="outline" 
          ...
          >
            ...
            {isInitializing ? "Initializing..." : "Re-Initialize System Nodes"}
          </Button>
        </div>
      </div>
